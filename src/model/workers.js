...
const selectAllWorker = (limit, offset, searchParam, sortBY, sort) => {
  ...
  return Pool.query(
    `SELECT * FROM workers WHERE name ILIKE '%${searchParam}%' ORDER BY ${sortBY} ${sort} LIMIT ${limit} OFFSET ${offset} `
    // ILIKE = not case sensitive, LIKE = case sensitive
  );
};
...
const updateWorker = (data) => {
  const { id, name, phone, jobdesk, address, workplace, description, image } =
    data;
  return Pool.query(
    `UPDATE workers SET name='${name}',phone='${phone}',jobdesk='${jobdesk}',description='${description}',address='${address}',workplace='${workplace}', image='${image}' WHERE id_worker='${id}'`
  );
};
...
const findId = (id) => {
  return new Promise((resolve, reject) =>
    Pool.query(
      `SELECT id_worker FROM workers WHERE id_worker='${id}'`,
      (error, result) => {
        if (!error) {
          resolve(result);
        } else {
