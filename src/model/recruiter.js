...
const selectAllRecruiter = (limit, offset, searchParam, sortBY, sort) => {
  return Pool.query(
    `SELECT * FROM recruiters WHERE name LIKE '%${searchParam}%' ORDER BY ${sortBY} ${sort} LIMIT ${limit} OFFSET ${offset} `
  );
};
...
const findId = (id) => {
  return new Promise((resolve, reject) =>
    Pool.query(
      `SELECT id_recruiter FROM recruiters WHERE id_recruiter='${id}'`,
      (error, result) => {
        if (!error) {
          resolve(result);
        } else {
...
const registerRecruiter = (data) => {
  ...
  return Pool.query(
    `INSERT INTO recruiters(id_recruiter,name,email,company_name,position,phone,job_field,city,description,instagram,linkedin, password, role) VALUES('${id}','${name}','${email}','${company_name}','${position}','${phone}','${job_field}','${city}','${description}','${instagram}','${linkedin}','${password}','${role}')`
  );
};
