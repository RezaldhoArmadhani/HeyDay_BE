...
const updateRecruiter = (data) =>{
    ...
    return Pool.query(`UPDATE recruiters SET fullname='${name}',email='${email}',company_name='${company_name}',position='${position}',phone='${phone}',job_field='${job_field}',city='${city}',description_company='${description}', instagram='${instagram}',linkedin='${linkedin}', password='${password}' WHERE id_recruiter='${id}'`);
}
...
const registerRecruiter = (data) => {
  const { id,name,phone,email,password,position, job_field, city, description, instagram, linkedin, company_name, role} = data;
    
  return Pool.query(`INSERT INTO recruiters(id_recruiter,fullname,email,company_name,position,phone,job_field,city,description_company, instagram,linkedin, password, role) VALUES('${id}','${name}','${email}','${company_name}','${position}','${phone}','${job_field}','${city}','${description}','${instagram}','${linkedin}','${password}','${role}')`);
}
