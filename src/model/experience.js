...
const selectAllExperience = (limit, offset, searchParam, sortBY, sort) => {
  return Pool.query(
    `SELECT * FROM experiences WHERE company_name LIKE '%${searchParam}%' ORDER BY ${sortBY} ${sort} LIMIT ${limit} OFFSET ${offset} `
  );
};

const showExperienceByUserId = (id_worker) => {
  return Pool.query(
    `SELECT * FROM experiences WHERE id_worker = '${id_worker}'`
  );
};
...
const updateExperience = (data) => {
  const { id, id_worker, company, jobdesk, startdate, enddate, description } =
    data;
  return Pool.query(
    `UPDATE experiences SET company_name='${company}', jobdesk='${jobdesk}', date_start='${startdate}', date_end='${enddate}', description='${description}' WHERE id_worker='${id_worker}' and id_experience='${id}'`
  );
};

const deleteExperience = (id, id_worker) => {
  return Pool.query(
    `DELETE FROM experiences WHERE id_experience='${id}' and id_worker='${id_worker}'`
  );
};
...
const findId = (id) => {
  return new Promise((resolve, reject) =>
    Pool.query(
      `SELECT id_experience FROM experiences WHERE id_experience='${id}'`,
      (error, result) => {
        if (!error) {
          resolve(result);
        } else {
